# s33 System Optimization Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create s33-system-optimization SKILL.md following existing conventions, then update SKILLS-CATALOG.md, AI-AGENT-MAPPING.md, and CLAUDE.md to register the new skill.

**Architecture:** Single monolithic SKILL.md file under `skills/s33-system-optimization/` containing all 7 modules (M1-M7), execution protocol, input/output contracts, AI agent integration, dispatch rules, triggers, and success criteria. Follows the exact frontmatter and section structure of s31/s32.

**Tech Stack:** Markdown (SKILL.md), YAML (artifacts references), JSON (catalog entries)
...
|---|---|---|
| `skills/s33-system-optimization/SKILL.md` | Create | Complete skill definition with all 7 modules |
| `skills/SKILLS-CATALOG.md` | Modify | Add s33 row + update AI Agent Coverage + Cross-Reference + Autonomy tables |
| `skills/AI-AGENT-MAPPING.md` | Modify | Add s33 row in Strategic Innovation section + update last-updated date |
| `CLAUDE.md` | Modify | Add s33 to repo structure, workflow map, skill index table, and AI Agent Coverage table |

---

### Task 1: Create skills/s33-system-optimization/SKILL.md

**Files:**
- Create: `skills/s33-system-optimization/SKILL.md`

- [ ] **Step 1: Create the skill directory**

```bash
mkdir -p skills/s33-system-optimization
```

- [ ] **Step 2: Write the complete SKILL.md**

Create `skills/s33-system-optimization/SKILL.md` with the full content below. The file follows the exact frontmatter + section convention of s31/s32.
...
```bash
wc -l skills/s33-system-optimization/SKILL.md
head -5 skills/s33-system-optimization/SKILL.md
```

Expected: ~600+ lines, starts with `---` frontmatter

- [ ] **Step 4: Commit**

```bash
git add skills/s33-system-optimization/SKILL.md
git commit -m "feat: add s33 system optimization skill with 7 audit modules

L2 advisory + artifact generation skill covering latency deep-dive,
N+1 detection, CCU stress testing, atomicity verification, concurrency
auditing, security vulnerability auditing, and agent-proposed evaluations.

Callable at any phase. Supports full/targeted/single execution modes."
```

---

### Task 2: Update skills/SKILLS-CATALOG.md

**Files:**
- Modify: `skills/SKILLS-CATALOG.md`

- [ ] **Step 1: Add s33 row to Skill Catalog table**

After the `| s32 | deep-research | Any | L1/L1 | None (research by design) |` line, add:

```
| s33 | system-optimization | Any | L2/L2 | Harness AI Test Agent (M1/M3), AppSec/STO Agent (M6), SRE Agent (M4/M5), DevOps Agent (M2) |
```

- [ ] **Step 2: Add to AI Agent Coverage Matrix**

After the `| None (research by design) | s32 (L1/L1) |` line, add:
...
Note: This replaces the existing lines for these agents to add s33 coverage. Update the existing rows in-place.

- [ ] **Step 3: Add to Autonomy Level Distribution**

In the **Current Levels** L2 section, add s33:
```
| L2 | 14 | s02, s03, s04, s05, s08, s10, s11, s12, s23, s24, s25, s26, s27, s33 |
```
Update L1 count from 20 to 20 (s33 is L2, counts stay same for L1).

In the **Target Levels** L2 section, add s33:
```
| L2 | 19 | s01, s02, s06, s07, s09, s13, s14, s15, s16, s17, s18, s19, s20, s21, s22, s28, s29, s30, s33 |
```

- [ ] **Step 4: Add to Cross-Reference Map**

After the `| s32 deep-research | s00, s01, s02 | None |` line, add:

```
| s33 system-optimization | s00, s01, s02, s05, s11, s13 | s01, s04, s05, s11, s13, s14, s25, s26, s28, s31, s32 |
```

- [ ] **Step 5: Update total skills count**

Change `**Total skills**: 33` to `**Total skills**: 34`

- [ ] **Step 6: Commit**

```bash
git add skills/SKILLS-CATALOG.md
git commit -m "feat: register s33 in skills catalog with agent coverage"
```

---

### Task 3: Update skills/AI-AGENT-MAPPING.md

**Files:**
- Modify: `skills/AI-AGENT-MAPPING.md`

- [ ] **Step 1: Update the Last Updated date**

Change `**Last Updated**: 2026-05-27` to `**Last Updated**: 2026-05-28`

- [ ] **Step 2: Add s33 section after s32 in Strategic Innovation table**

After the s32 row in the "Strategic Innovation (s31-s32)" table, update the section header to "Strategic Innovation & Optimization (s31-s33)" and add:
...
```bash
git add skills/AI-AGENT-MAPPING.md
git commit -m "feat: add s33 agent mapping with multi-agent coverage"
```
...
- [ ] **Step 1: Add s33 to repository structure tree**

After the `s32-deep-research/` line, add:
...
- [ ] **Step 2: Add s33 to the workflow map**

After the s32 block in the workflow map, add:
...
- [ ] **Step 3: Add s33 row to Quick Reference: Skill Index table**

After the `| 32 | Deep Research | Any | research this, find papers, literature review, evidence for |` line, add:

```
| 33 | System Optimization | Any | optimize, latency, N+1, stress test, CCU, atomicity, race condition, security audit |
```

- [ ] **Step 4: Add s33 to AI Agent Coverage table**

After the `| Knowledge Graph | s03 (L2/L3) |` row, and updating existing agent rows to include s33. Specifically, add s33 rows:

```
| Test Agent (M1/M3), AppSec/STO Agent (M6), SRE Agent (M4/M5), DevOps Agent (M2) | s33 (L2/L2) |
```
...
```bash
git add CLAUDE.md
git commit -m "feat: register s33 system optimization in project CLAUDE.md"
```
...
```bash
ls -la skills/s33-system-optimization/SKILL.md
```
...
```bash
head -15 skills/s33-system-optimization/SKILL.md
```

Expected: starts with `---`, contains `name: system-optimization`, has `description:` block

- [ ] **Step 3: Verify catalog has s33 entry**

```bash
grep "s33" skills/SKILLS-CATALOG.md
```

Expected: at least 4 lines matching (catalog row, agent coverage, cross-reference, autonomy)

- [ ] **Step 4: Verify AI-AGENT-MAPPING has s33**

```bash
grep "s33" skills/AI-AGENT-MAPPING.md
```

Expected: at least 1 line with s33 entry

- [ ] **Step 5: Verify CLAUDE.md has s33**

```bash
grep "s33" CLAUDE.md
```

Expected: at least 3 lines (repo structure, skill index, AI coverage)

- [ ] **Step 6: Verify design spec status updated**

Update the design spec status from `Draft` to `Approved` in `docs/superpowers/specs/2026-05-28-s33-system-optimization-design.md`.

```bash
git add docs/superpowers/specs/2026-05-28-s33-system-optimization-design.md
git commit -m "docs: mark s33 design spec as approved"
```
